...
class FG_ApplyIPAdapter:
    ...
    CATEGORY = "Farrenzo's Garbage/IP Adapter/SDXL"
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "weight": (c_types.IO.FLOAT, { "default": 1.0, "min": -1, "max": 5, "step": 0.05 }),
    ...
    def _load_ipadapter_model(self, ipadapter_file):
        ipadapter_file = folder_paths.get_full_path("ipadapter", ipadapter_file)
        ipadapter_model = c_utils.load_torch_file(ipadapter_file, safe_load=True)
        if ipadapter_file.lower().endswith(".safetensors"):
            ...
            for key in ipadapter_model.keys():
                if key.startswith("image_proj."):
                    ...
                    st_model["ip_adapter"][key.replace("adapter_modules.", "")] = ipadapter_model[key]
            ipadapter_model = st_model
            del st_model
        elif "adapter_modules" in ipadapter_model.keys():
            ipadapter_model["ip_adapter"] = ipadapter_model.pop("adapter_modules")

        if not "ip_adapter" in ipadapter_model.keys() or not ipadapter_model["ip_adapter"]:
            raise Exception("invalid IPAdapter model {}".format(ipadapter_file))

        if 'plusv2' in ipadapter_file.lower():
            ipadapter_model["faceidplusv2"] = True

        if 'unnorm' in ipadapter_file.lower():
            ipadapter_model["portraitunnorm"] = True

        return ipadapter_model

    def _load_CLIP_Vision(self, clip_name):
        clip_path = folder_paths.get_full_path_or_raise("clip_vision", clip_name)
        clip_vision_model = c_visiz.load(clip_path)
