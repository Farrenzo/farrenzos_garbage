			...
			nodeType.prototype.onNodeCreated = function () {
				...
				CUSTOM_INT(
					...
					function (v, _, node) {
						...
						node.widgets[1].value = values[v][0]
						node.widgets[2].value = values[v][1]
						node.widgets[3].value = values[v][2]
					},
					{ step: 10, max: 1 }

				)

				CUSTOM_INT(this, "x", 0, function (v, _, node) {transformFunc(this, v, node, 0, node.widgets[node.indexWidget].value)}, {step: 80})
				CUSTOM_INT(this, "y", 0, function (v, _, node) {transformFunc(this, v, node, 1, node.widgets[node.indexWidget].value)}, {step: 80})
				CUSTOM_INT(this, "feather", 1, function (v, _, node) {transformFunc(this, v, node, 2, node.widgets[node.indexWidget].value)}, {"min": 0.0, "max": 4096, "step": 80, "precision": 0})
					...
					options.unshift(
						{
							...
							callback: () => {
								...
								this.widgets[this.indexWidget].options.max = inputLenth-1
