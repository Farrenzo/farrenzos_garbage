...
export function transformFunc(widget, value, node, index, rowIndex) {
	const s = widget.options.step / 10;
	widget.value = Math.round(value / s) * s;
	node.properties["values"][rowIndex][index] = widget.value
}
...
export function removeNodeInputs(node, indexesToRemove, offset=0) {
	...
	node.widgets[node.indexWidget].options.max = inputLenght
	if (node.widgets[node.indexWidget].value > inputLenght) {
		node.widgets[node.indexWidget].value = inputLenght
	}
