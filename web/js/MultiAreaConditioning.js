...
function addMultiAreaConditioningCanvas(node, app) {
	...
	const widget = {
		...
		draw: function (ctx, node, widgetWidth, widgetY) {
			...
			const index = Math.round(node.widgets[node.indexWidget].value)
...
app.registerExtension({
	...
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		if (nodeData.name === "MultiAreaConditioning") {
			...
			nodeType.prototype.onNodeCreated = function () {
				...
				CUSTOM_INT(this, "x", 0, function (v, _, node) {transformFunc(this, v, node, 0, node.widgets[node.indexWidget].value)})
				CUSTOM_INT(this, "y", 0, function (v, _, node) {transformFunc(this, v, node, 1, node.widgets[node.indexWidget].value)})
				CUSTOM_INT(this, "width", 0, function (v, _, node) {transformFunc(this, v, node, 2, node.widgets[node.indexWidget].value)})
				CUSTOM_INT(this, "height", 0, function (v, _, node) {transformFunc(this, v, node, 3, node.widgets[node.indexWidget].value)})
				CUSTOM_INT(this, "strength", 1, function (v, _, node) {transformFunc(this, v, node, 4, node.widgets[node.indexWidget].value)}, {"min": 0.0, "max": 10.0, "step": 0.1, "precision": 2})

				// Canvas must be added LAST so ComfyUI positions input slots correctly
				addMultiAreaConditioningCanvas(this, app)

				this.getExtraMenuOptions = function(_, options) {
					options.unshift(
						{
							content: `insert input above ${this.widgets[this.indexWidget].value} /\\`,
							callback: () => {
								this.addInput("conditioning", "CONDITIONING")
								
								const inputLenth = this.inputs.length-1
								const index = this.widgets[this.indexWidget].value
								...
								this.widgets[this.indexWidget].options.max = inputLenth
						...
						{
							content: `insert input below ${this.widgets[this.indexWidget].value} \\/`,
							callback: () => {
								this.addInput("conditioning", "CONDITIONING")
						...
						{
							content: `swap with input above ${this.widgets[this.indexWidget].value} /\\`,
							callback: () => {
								const index = this.widgets[this.indexWidget].value
								if (index !== 0) {
									...
									this.widgets[this.indexWidget].value = index-1
						...
						{
							content: `swap with input below ${this.widgets[this.indexWidget].value} \\/`,
							callback: () => {
								const index = this.widgets[this.indexWidget].value
						...
						{
							content: `remove currently selected input ${this.widgets[this.indexWidget].value}`,
							callback: () => {
								const index = this.widgets[this.indexWidget].value
	...
	loadedGraphNode(node, _) {
		if (node.type === "MultiAreaConditioning") {
			node.widgets[node.indexWidget].options["max"] = node.properties["values"].length-1
		}
	},
